...
# Import the data 
HTrees4 <- read.table(file = "abundance.natural.and.mimics.txt",
                      header = TRUE,
                      dec = ".")
...
#p2 <- p2 + ylab(expression(paste('Sedimentation Rate ('*'mg'~ cm^-2~d^-1*')')))
p2 <- p2 + xlab(expression(paste('treatment')))
p2 <- p2 + theme(text = element_text(size=2)) + theme_bw()
p2 <- p2 + scale_x_discrete(labels = c('tree','mimic'))
...
treatplot2 <- ggplot(dat5, aes(x = x, y = predicted)) +
  geom_jitter(data = filter(HTrees4, Treatment=="a.real.tree"), aes(y = Abundance, x = Treatment), 
              size = 4, height = 0.01, width = 0.1, alpha = 0.7, colour = "forestgreen") +
  geom_jitter(data = filter(HTrees4, Treatment=="c.mimic"), aes(y = Abundance, x = Treatment), 
